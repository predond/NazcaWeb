namespace NazcaWeb.Models
{
    public class IRC
    {
        ...
        public static string StartPath = @"\\DISKSTATION\video1\Filmy Dawid\Private\";
        ...
        public bool BrowseForVideo(VideoItem video, bool startTV, bool startFromVT, Action<string, bool, bool> onSegmentProcessed)
        {
            Task.Run(async () =>
            {
                ...
                foreach (var target in video.FullPath.Replace("\\\\", "\\").Replace(processedPath, "").Split('\\'))
                {
        ...
        private async void ReturnToOriginalPathAsync()
        {
            ...
            foreach (var segment in path)
            {
                ...
                await Task.Delay(750, returnCancellationToken);
